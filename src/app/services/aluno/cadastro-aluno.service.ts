...
export class CadastroAlunoService {
    ...
    async getData() {
      try {
        ...
        if (user) {
          this.db.ref('Alunos').child(user.uid).on('value', (snapshot) => {
    ...
    validFormData() {
      if (this.formAluno.valid) {
        ...
        this.saveData(currentID ?? this.fireDatabase.createPushId());
      } else {
    ...
    async saveData(id: string) {
      try {
        const user = await this.fireAuth.currentUser;
        if (user?.uid) {
          await this.db.ref('Alunos').child(user.uid).child(id).update(this.formAluno.value);
